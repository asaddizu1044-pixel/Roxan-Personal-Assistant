...
export function usePhoneSensors() {
  ...
  const onMotion = useCallback(
    (event: DeviceMotionEvent) => {
      ...
      motionSamples.current = rawSamples;

      const smoothedMotion =
        rawSamples.reduce((sum, v) => sum + v, 0) / rawSamples.length;

      const average =
        rawSamples.reduce((sum, value) => sum + value, 0) /
        rawSamples.length;

      const peak = Math.max(...rawSamples);

      const now = Date.now();

      const isPeak =
        previousMotionValue.current > previousPreviousMotionValue.current &&
        previousMotionValue.current > smoothedMotion &&
      ...
      previousMotionValue.current = smoothedMotion;
      lastMotionValue.current = smoothedMotion;
...
function estimateStepsFromDistance(
  distanceMeters: number,
  stride: number,
) {
...
export function usePhoneSensors() {
  ...
  const onMotion = useCallback(
    (event: DeviceMotionEvent) => {
      ...
      // Use GPS-based step estimation if motion steps are 0
      const estimatedSteps = estimateStepsFromDistance(
        gpsDistance.current,
        DEFAULT_STRIDE_METERS,
      );

      const finalSteps = stepCount.current > 0 ? stepCount.current : estimatedSteps;
